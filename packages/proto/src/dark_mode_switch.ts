...
export class dark_mode_switch_Element extends LitElement {
  @property()
  href?: string;
  ...
  protected firstUpdated() {
    const checkbox =
      this.renderRoot.querySelector<HTMLInputElement>("#checkbox");
    if (!checkbox) return;
    ...
    const syncFromSystem = (event: MediaQueryList | MediaQueryListEvent) => {
      const matches =
    ...
    syncFromSystem(browserTheme);

    browserTheme.addEventListener("change", syncFromSystem);
  }
}
